...
describe League, type: :model do
  describe '.header_blurred_image' do
    ...
    context 'includes all files in the specified folder' do
      ...
      it { expect(league.header_blurred_images.values.first).to eq('https://localhost:3000/images/nhl-teams%2Fblurred-headers%2Fchi.png') }
    end
  end
  describe '.header_image' do
    ...
    context 'includes all files in the specified folder' do
      ...
      it { expect(league.header_images.values.first).to eq('https://localhost:3000/images/nhl-teams%2Fheaders%2Fchi.png') }
    end
  end
  describe '.scores' do
    let(:league) { League.new('nhl') }
